...
function getNextFriday(date) {
  ...
  const dayFriday = (5 - day.getDay() + 7) % 7;
  if (dayFriday === 0) {
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dayS = new Date(dateStart);
  const dayE = new Date(dateEnd);
  const daySSecond = dayS.getTime();
  const dayESecond = dayE.getTime();
  const diff = dayESecond - daySSecond;
  const diffDay = diff / (1000 * 60 * 60 * 24);
  const diffDayPlus = diffDay + 1;
  return diffDayPlus;
}
...
function isDateInPeriod(date, period) {
  ...
  const dateMil = dateDate.getTime();
  const periodS = new Date(period.start);
  const periodE = new Date(period.end);
  const periodSMil = periodS.getTime();
  const periodEMil = periodE.getTime();
  if (dateMil >= periodSMil && dateMil <= periodEMil) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  const dateDate = new Date(date);
  return dateDate.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  const dateDate = new Date(date);
  dateDate.setDate(dateDate.getDate() + 4 - (dateDate.getDay() || 7));
  const firstDayYear = new Date(dateDate.getFullYear(), 0, 1);
  const firstDYMil = firstDayYear.getTime();
  const dateMil = dateDate.getTime();
  const diff = dateMil - firstDYMil;
  const day = diff / (1000 * 60 * 60 * 24);
  const numberWeek = Math.ceil((day + 1) / 7);
  return numberWeek;
}
...
function getNextFridayThe13th(date) {
  const dateDate = new Date(date);
  const limitYear = dateDate.getFullYear() + 10;
  const friday = new Date(dateDate.getFullYear(), dateDate.getMonth(), 13);
  for (let i = 0; i <= limitYear; i += 1) {
    if (friday > dateDate && friday.getDay() === 5) {
      return friday;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const [sDay, sMonth, sYear] = period.start.split('-').map(Number);
  const [eDay, eMonth, eYear] = period.end.split('-').map(Number);
  const startD = new Date(sYear, sMonth - 1, sDay);
  const endD = new Date(eYear, eMonth - 1, eDay);
  const result = [];
  const current = new Date(startD);
  while (current <= endD) {
    for (let i = 0; i < countWorkDays && current <= endD; i += 1) {
